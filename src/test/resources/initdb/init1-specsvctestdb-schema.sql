...
CREATE TABLE IF NOT EXISTS SpecSvcSchema.BatteryTypes (
  ...
  CONSTRAINT terminal_layout_id FOREIGN KEY (terminal_layout_id) REFERENCES TerminalLayouts(terminal_layout_id) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT battery_tier_id FOREIGN KEY (battery_tier_id) REFERENCES BatteryTiers(battery_tier_id) ON DELETE NO ACTION ON UPDATE NO ACTION
);