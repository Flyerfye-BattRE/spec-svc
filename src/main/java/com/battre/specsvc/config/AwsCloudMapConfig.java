package com.battre.specsvc.config;
...
public class AwsCloudMapConfig {
    ...
    public void registerService() {
        // Svcs and insts only need to be registered for local container insts since AWS:ECS does it automatically
    ...
    public String createService() {
        ...
        DnsConfig dnsConfig = DnsConfig.builder()
        ...
        CreateServiceResponse createServiceResponse = serviceDiscoveryClient.createService(createServiceRequest);
    ...
    public void registerInstance(String serviceId) {
        ListInstancesResponse listInstancesResponse =
                serviceDiscoveryClient.listInstances(ListInstancesRequest.builder().serviceId(serviceId).build());
        ...
        for (InstanceSummary instanceSummary : listInstancesResponse.instances()) {
            if (instanceSummary.id().equals(AWS_INSTANCE)) {
                logger.info("Found an instance of [" + AWS_INSTANCE + "]: Deleting before creating new instance");
        ...
        RegisterInstanceRequest registerInstanceRequest = RegisterInstanceRequest.builder()
                .serviceId(serviceId)
                .instanceId(AWS_INSTANCE)
                .attributes(Map.of(
        ...
        RegisterInstanceResponse registerInstanceResponse = serviceDiscoveryClient.registerInstance(registerInstanceRequest);

        System.out.println("Service registered with instance ID: " + registerInstanceResponse.operationId());
    }
}